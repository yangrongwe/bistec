...
async function processQueue() {
  ...
  try {
    ...
    console.error("发送队列任务执行失败:", error);
  } finally {
    isSending = false;
    // 处理下一个任务
    processQueue();
...
export function sendData(modeIndex, that) {
  ...
  const sendTask = async () => {
    console.log("发送的数据", that);
    // 当前模式
    var mode = modeIndex; // 位2~1
    // 拉压/侧倾/前悬架/后悬架强度（1-9级）
    var la_ya = that.drawProgress || 1; // 拉压强度 位6~3
    var hou_xuan_jia =
      (that.gaugeList && that.gaugeList[2] && that.gaugeList[2].progress) || 1; // 后悬架强度 位10~7
    var ce_qing =
      (that.gaugeList && that.gaugeList[1] && that.gaugeList[1].progress) || 1; // 侧倾强度 位14~11
    var qian_xuan_jia =
      (that.gaugeList && that.gaugeList[0] && that.gaugeList[0].progress) || 1; // 前悬架强度 位18~15
    // 快速调节
    ...
    console.log(
      "分段表示: 激活标志位(23-21):",
      (data >> 21).toString(2).padStart(3, "0"),
      "快速调节(20-19):",
      ((data >> 19) & 0x03).toString(2).padStart(2, "0"),
      "前悬架(18-15):",
      ((data >> 15) & 0x0f).toString(2).padStart(4, "0"),
      "侧倾(14-11):",
      ((data >> 11) & 0x0f).toString(2).padStart(4, "0"),
      "后悬架(10-7):",
      ((data >> 7) & 0x0f).toString(2).padStart(4, "0"),
      "拉压(6-3):",
      ((data >> 3) & 0x0f).toString(2).padStart(4, "0"),
      "模式(2-1):",
      (data & 0x03).toString(2).padStart(2, "0"),
    );
    // 验证各参数的计算是否正确
    const baseData =
      mode +
    ...
    console.log(
      "计算验证: baseData + activateFlagValue =",
      baseData + (activateFlag === 1 ? 1 << 20 : 0),
    );
    ...
    try {
      const res = await bluetoothManager.sendData(buff.buffer);
